...
### Lead-lag evidence: prediction markets lead equity repricing

The table below summarises the lead-lag relationship between Polymarket
prediction market contracts and US equity/ETF prices around macro events.
Three independent methods are applied to each event window (T-72h to
T+72h at 1-min resolution): path signature signed area, cross-correlation,
and Granger causality.

| Event Type | N Events | Signed Area (mean) | % PM Leads | Granger p-value |
|-----------|----------|-------------------|------------|-----------------|
| **CPI**   | 15       | +0.134            | **100%**   | 0.529           |
| **NFP**   | 10       | +0.282            | **80%**    | < 0.001         |
| **FOMC**  | 25       | +0.071            | **72%**    | < 0.001         |
| **GDP**   | 100      | +0.036            | **60%**    | < 0.001         |

**Key findings:**
- **CPI releases show a perfect 15/15 lead** -- prediction markets
  reprice before equities on every single CPI event in the sample.
  This is the strongest evidence for the thesis.
- **NFP produces the largest signed area** (+0.282), consistent with
  payroll surprises generating sharp, fast repricing in prediction
  markets that takes longer to propagate through equity order books.
- **FOMC and GDP** show weaker but still directionally positive signals
  (72% and 60% of events show PM leading).  FOMC is highly statistically
  significant via Granger causality (p < 0.001).
- PCE and PMI are not yet covered due to limited Polymarket contract
  availability for these event types.

> *Reproduced by running `python -m src.analysis.run_analysis -v`. Full
> results in `results/tables/lead_lag_summary.csv`.*

### Signed area distribution

The signed area (level-2 path signature cross-term) captures lead-lag
directionality in a single number: **positive = prediction market moved
first**, negative = equity moved first.  A distribution shifted right
confirms the thesis.

![Signed area distribution](results/figures/signed_area_distribution.png)

### Lead-lag by event type

Box plot showing which macro event types produce the strongest and most
consistent prediction market lead.  Green = median positive (PM leads),
red = median negative.

![Signed area by event type](results/figures/signed_area_by_event_type.png)

### Event study: CPI release

Average prediction market probability and equity cumulative return
around CPI releases, aligned at T=0 (release time).  The prediction
market reprices sharply at T=0 while the equity response builds over
the following minutes.

![Event study CPI](results/figures/event_study_cpi.png)

### Cross-correlation function

Lagged cross-correlation between prediction market probability changes
and equity returns.  A peak at positive lag confirms that prediction
market moves precede equity moves by that many minutes.

![Cross-correlation](results/figures/cross_correlation_all_events.png)

### Regime detection via TDA

Persistent homology (H1 features) applied to sliding windows of the
joint (pred_mkt, equity) point cloud.  Spikes in H1 persistence
correspond to periods with strong cyclic lead-lag structure.  Red dots
mark detected regime changes.

![Regime timeline](results/figures/regime_timeline_all_events.png)

> *All figures are generated by running:*
> ```bash
> pip install -e ".[analysis]"
> python -m src.analysis.run_analysis -v
> ```
